...
- **Modern_SOF_Vehicles**: `isv` (9-seat, unarmed), `rsov_m240`/`rsov_m2`/`rsov_mk19` (M249 pintle on all), `mrzr2`/`mrzr4` (M249 pintle, diesel_engine_inline4), `flyer` (ATGM turret + M240 pintle), `gmv` (M1288: Mk19 + HEL-AD laser + 6 batteries + 6 solar + non-reactor generator hardware; NOT in spawn groups — debug-spawn only). Custom parts: `turret_rmes_laser` (HEL-AD), `hel_apu`.
