...
- **Vehicle power/REACTOR**: the only working `REACTOR` part is the appliance gas generator. Field semantics (per its own comment): `epower` = conversion speed, `power` = fuel efficiency (fuel mL : charges). Our GMV APU (`hel_apu`) burns the vehicle's diesel/JP8 to charge batteries. TLG has NO plutonium/reactor vehicle part (plut_cell is inert ammo). Solar panels in TLG all output **50 W regardless of tier** (advanced/reinforced only differ in durability). Battery capacity: `large_storage_battery` = 100,000 kJ, `storage_battery` = 50,000 kJ, 1 unit ≈ 1 kJ.
